...
class BleakServerDotNet(BaseBleakServer):
    ...
    async def add_new_characteristic(self, service_uuid: str, char_uuid: str, properties: GattCharacteristicsFlags, value: bytearray, permissions: int):
        ...
        characteristicResult = await wrap_IAsyncOperation(
                IAsyncOperation[GattLocalCharacteristicResult](
                        self.services.get_service(str(serverguid)).obj.CreateCharacteristicAsync(charguid, ReadParameters)
                    ),
        ...
        self.services.get_service(str(serverguid)).add_characteristic(bleak_characteristic)
    
        self.services.add_characteristic(bleak_characteristic)
    
    #@staticmethod
    def _read_characteristic(self, sender: GattLocalCharacteristic, args: GattReadRequestedEventArgs):
    ...
    #@staticmethod        
    def _write_characteristic(self, sender: GattLocalCharacteristic, args: GattWriteRequestedEventArgs):
        ...
        logger.debug("senderuuid : {}".format(self.services.get_characteristic(str(sender.Uuid)).value))
        self.services.get_characteristic(str(sender.Uuid)).value = value 
        logger.debug("HOORRAAYYY")
    ...
    def _get_request(self, args: Union[GattReadRequestedEventArgs, GattWriteRequestedEventArgs]):
        
        request = Request()
        logger.debug("Attempting to obtain request: {}".format(request._obj))
        
        logger.debug("Starting Thread")
        fun_thread = Thread(target=self._run_request_thread, args=[args, request])
        fun_thread.start()
        ...
        fun_thread.join()
        logger.debug("Thread finished. Request result: {}".format(request._obj))
        return request._obj
    
    def _run_request_thread(self, args: Union[GattReadRequestedEventArgs, GattWriteRequestedEventArgs], request: Request):
    ...
    async def _request_thread_loop(self, args: Union[GattReadRequestedEventArgs, GattWriteRequestedEventArgs], request: Request):
